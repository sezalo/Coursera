...
summary(multifit)
```
With a multivariate model, the residual variation its decreased as evidenced in the $R^2=0.8303$ obtained. Also note by the significance levels, that the \textit{gpm} its more strongthly dependant in \textit{cyl} and \textit{wt} rather than in \textit{am}. As well, \textit{am} seems not to be independat variable as its coeficient changes with the presence of ther other variables to $\beta_{am}=0.17$. A variance and resisual analysis can be found the Appendix.

\pagebreak
# APPENDIX 
```{r Append, echo=FALSE}
#Variance analysis
print("A. VARIANCE ANALYSIS")
anova(multifit)

#Residual plots
print("B. RESIDUAL PLOTS")
par(mfrow = c(2, 2))
plot(multifit)

#Scatterplot matrix of the dataset:
print("C.DATASET SCATTERPLOT DATASET ")
